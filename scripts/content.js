...
function createHTMLElem(gptAnswer){


    
    const answerArea = document.createElement("div");//biggest box that holds everything
    answerArea.id = "answerArea";
    answerArea.style.fontFamily = "Google Sans,arial,sans-serif";
    answerArea.style.backgroundColor = "#ffffff";
    answerArea.style.border = "2px solid #bababa";
    ...
    if (resultStatsElement) {
        resultStatsElement.insertBefore(answerArea, resultStatsElement.firstChild);
    }
    console.log("added box");

    var style = document.createElement('style');
    answerArea.appendChild(style);
...
async function callGptAgain(text, gptAnswer){
    console.log("Calling GPT API...");
...
(async() => {
    ...
    if (searchBox) {
        ...
        answerBox.innerText = "";
        const words = gptAnswer.split(" ");//array
        for (let word of words){
            answerBox.innerText += " " + word;
            await delay(30);
        }
        ...
        generateMoreButton.onclick = async function() {
            ...
            const gptNewAnswer = await callGptAgain(text, gptAnswer);

            generateMoreButton.style.display='none';
            
            const newWords = gptNewAnswer.split(" ");//array
